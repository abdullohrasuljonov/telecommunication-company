...
import uz.pdp.communicationsystem.component.SerialNumberGenerator;
...
import uz.pdp.communicationsystem.payload.SimCardDto;
import uz.pdp.communicationsystem.repository.*;
...
public class SimCardService {
    ...
    public ApiResponse connectTariff(UUID tariffId) {
        SimCard simCard = (SimCard) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        Optional<Tarif> tariffRepositoryById = tariffRepository.findById(tariffId);
        if (!tariffRepositoryById.isPresent()) return new ApiResponse("Tarif mavjud emas", false);
        Tarif tariff = tariffRepositoryById.get();
